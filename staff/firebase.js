...
import { getFirestore, where, getDoc, collection, getDocs, query,orderBy } from 'https://www.gstatic.com/firebasejs/9.22.0/firebase-firestore.js';
...
auth.onAuthStateChanged(async function (user) {
  if (user) {
    ...
    const querySnapshot = await getDocs(query(dataCollectionRef, orderBy("timestamp", "desc")));
    let count = 1;
console.log(querySnapshot)
    for (const doc of querySnapshot.docs) {
      ...
      row.insertCell(11).innerHTML = `<a href="${pdfDownloadURL}"><img src="../images/open.gif" alt="Your GIF"></a>`;
